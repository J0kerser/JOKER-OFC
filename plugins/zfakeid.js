...
Asena.addCommand({pattern: 'fakeid ?(.*)', fromMe: true, desc: '' , dontAddCommandList: true }, async (message, match) => {
...
Asena.addCommand({pattern: 'fakeid ?(.*)', fromMe: false, desc: '' , dontAddCommandList: true }, async (message, match) => {
	const url = `https://docs-jojo.herokuapp.com/api/fake_identity?lang=en&type=json`;
	try {
		const response = await got(url);
